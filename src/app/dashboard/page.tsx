...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // if you use bearer tokens, we read from localStorage (you were already doing this)
      ...
      const [plansRes, servicesRes, subscriptionRes, usageRes] = await Promise.all([
        ...
        fetch(
          `/api/usage/summary?userId=${session?.user?.id}&month=${new Date().toISOString().slice(0, 7)}`,
          { headers: token ? { Authorization: `Bearer ${token}` } : undefined }
        )
      ]);

      // parse responses (defensive)
      ...
      <div className="relative z-10 container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          >
            ...
            <p className="text-3xl font-bold neon-gradient-text">{totalUsage.toLocaleString()}</p>
            <p className="text-xs text-gray-500 mt-1">of {totalLimit.toLocaleString()} this month</p>
          </motion.div>
          ...
          >
            ...
            <p className="text-3xl font-bold neon-gradient-text">{usageSummary.length}</p>
            <p className="text-xs text-gray-500 mt-1">of {services.length} available</p>
          </motion.div>
          ...
          >
            ...
            <p className="text-3xl font-bold neon-gradient-text">{currentPlan?.name || "Free"}</p>
            <p className="text-xs text-gray-500 mt-1">${currentPlan?.price || 0}/month</p>
          </motion.div>
        </div>
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between text-xs">
                      <span className="text-gray-500">{(percentage || 0).toFixed(1)}% used</span>
                      <span className="text-gray-500">
                        {usage ? usage.remainingUsage.toLocaleString() : service.monthlyLimit.toLocaleString()}{" "}
                        remaining
                      </span>
                    </div>
                  </div>
        ...
        >
          <div className="text-center mb-12">
            ...
            <p className="text-gray-400 max-w-2xl mx-auto">
              Scale your automation with flexible pricing options designed for every business stage
            </p>
          </div>
              ...
              return (
                <motion.div
                  ...
                  className={`relative glassmorphism rounded-xl border transition-all duration-300 ${
                    isCurrentPlan ? "border-cyan-500 shadow-lg shadow-cyan-500/20" : "border-white/10 hover:border-white/20"
                  } ${isPro ? "scale-105" : ""}`}
                >
                  {/* Popular Badge */}
                  {isPro && (
                  ...
                  <div className="p-6">
                    ...
                    <div className="mb-6">
                      <div className="flex items-baseline gap-2">
                        <span className="text-4xl font-bold neon-gradient-text">${plan.price}</span>
                        <span className="text-gray-400">/month</span>
                      </div>
                      {plan.price === 0 && <p className="text-sm text-gray-500 mt-1">Perfect to get started</p>}
                      {plan.name === "Starter" && <p className="text-sm text-gray-500 mt-1">For growing businesses</p>}
                      {plan.name === "Pro" && <p className="text-sm text-gray-500 mt-1">Most popular choice</p>}
                      {plan.name === "Enterprise" && <p className="text-sm text-gray-500 mt-1">Full automation ownership</p>}
                    </div>
          ...
          >
            <p className="text-gray-400 text-sm">All plans include 24/7 support • Cancel anytime • No hidden fees</p>
          </motion.div>
        </motion.div>
        ...
        {currentPlan && (
          ...
          >
            <h2 className="text-2xl font-bold mb-4">
              Current Plan: <span className="neon-gradient-text">{currentPlan.name}</span>
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col justify-center">
                <div className="text-center p-6 bg-gradient-to-r from-purple-500/10 to-cyan-500/10 rounded-lg border border-white/5">
                  ...
                  <p className="text-4xl font-bold neon-gradient-text mb-2">${currentPlan.price}</p>
