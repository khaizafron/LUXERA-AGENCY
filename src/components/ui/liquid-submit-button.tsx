"use client"

import * as React from "react"
import { cn } from "@/lib/utils"

export function LiquidSubmitButton({
  loading = false,
  children,
  className,
  ...props
}: React.ButtonHTMLAttributes<HTMLButtonElement> & {
  loading?: boolean
}) {
  const [isPressed, setIsPressed] = React.useState(false)
  const [isHovered, setIsHovered] = React.useState(false)

  return (
    <button
      {...props}
      disabled={loading || props.disabled}
      onMouseDown={() => setIsPressed(true)}
      onMouseUp={() => setIsPressed(false)}
      onMouseLeave={() => {
        setIsPressed(false)
        setIsHovered(false)
      }}
      onMouseEnter={() => setIsHovered(true)}
      className={cn(
        "relative w-full h-12 rounded-xl font-semibold text-lg transition-all duration-300 overflow-hidden select-none",
        "disabled:opacity-50 disabled:cursor-not-allowed",
        className
      )}
      style={{
        transform: isPressed ? "scale(0.97)" : "scale(1)",
        filter: isHovered && !isPressed ? "brightness(1.05)" : "none",
      }}
    >
      {/* Shadow and Glass */}
      <div
        className="
          absolute top-0 left-0 w-full h-full rounded-xl z-0
          shadow-[0_0_6px_rgba(0,0,0,0.03),0_2px_6px_rgba(0,0,0,0.08),
          inset_3px_3px_0.5px_-3px_rgba(0,0,0,0.9),
          inset_-3px_-3px_0.5px_-3px_rgba(0,0,0,0.85),
          inset_1px_1px_1px_-0.5px_rgba(0,0,0,0.6),
          inset_-1px_-1px_1px_-0.5px_rgba(0,0,0,0.6),
          inset_0_0_6px_6px_rgba(0,0,0,0.12),
          inset_0_0_2px_2px_rgba(0,0,0,0.06),
          0_0_12px_rgba(255,255,255,0.15)]
        "
      />

      <div
        className="absolute inset-0 rounded-xl -z-10"
        style={{ backdropFilter: 'url("#container-glass")' }}
      />

      {/* Foreground */}
      <div className="relative z-10 flex items-center justify-center gap-2">
        {children}
      </div>

      <GlassFilter />
    </button>
  )
}

function GlassFilter() {
  return (
    <svg className="hidden">
      <defs>
        <filter
          id="container-glass"
          x="0%"
          y="0%"
          width="100%"
          height="100%"
          colorInterpolationFilters="sRGB"
        >
          <feTurbulence
            type="fractalNoise"
            baseFrequency="0.05 0.05"
            numOctaves="1"
            seed="1"
            result="turbulence"
          />

          <feGaussianBlur in="turbulence" stdDeviation="2" result="blurredNoise" />

          <feDisplacementMap
            in="SourceGraphic"
            in2="blurredNoise"
            scale="70"
            xChannelSelector="R"
            yChannelSelector="B"
            result="displaced"
          />

          <feGaussianBlur in="displaced" stdDeviation="4" result="finalBlur" />

          <feComposite in="finalBlur" operator="over" />
        </filter>
      </defs>
    </svg>
  )
}
